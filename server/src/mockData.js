...
export function buildRecipe({ cut = 'אסאדו מעושן', method = 'מעשנה', effort = 'מאוזן', kosherPreference = 'כשר', seasoningStyle = 'קלאסי' } = {}) {
  ...
  const namedCut = withArticle(cleanCut);
  const isSmoker = method.includes('מעשנה');
  const isPot = method.includes('קדירה');
  const isMangal = method.includes('מנגל') || method.includes('גריל פחמים');
  const isQuick = effort === 'מהיר';
  const isKebabLamb = cleanCut.includes('קבב טלה');
  const kosherNote = kosherPreference === 'כשר' ? 'המתכון נשאר כשר וללא שילוב חלב ובשר.' : 'בגרסה לא כשרה אפשר להשתמש ברכיב עשיר רק אם הוא באמת מתאים לשיטה.';
  ...
  const sauceSets = [
    [
      { title: 'צ׳ימיצ׳ורי עשבי תיבול', description: 'רוטב רענן וחומצי שמאזן שומן ועשן.', ingredients: ['כוס פטרוזיליה קצוצה', '2 כפות כוסברה קצוצה', '2 שיני שום', '3 כפות שמן זית', 'כף חומץ או לימון', 'מלח ופלפל'], steps: ['קוצצים דק את העשבים והשום.', 'מערבבים עם שמן זית, חומץ, מלח ופלפל.', 'נותנים לרוטב לנוח 10 דקות ומגישים לצד הבשר.'] },
      { title: 'ברביקיו מעושן-מתקתק', description: 'רוטב סמיך שמתאים במיוחד לנתחים בבישול ארוך.', ingredients: ['4 כפות קטשופ איכותי', 'כף סילאן', 'כף חומץ תפוחים', 'כפית פפריקה מעושנת', 'חצי כפית חרדל'], steps: ['מערבבים את כל הרכיבים בסיר קטן.', 'מחממים 5 דקות על אש נמוכה עד הסמכה.', 'מברישים בסוף הצלייה או מגישים בצד.'] },
    ],
    [
      { title: 'טחינה חריפה עם לימון כבוש', description: 'טחינה חדה שמרימה בשרים צלויים בלי להכביד.', ingredients: ['חצי כוס טחינה גולמית', 'רבע כוס מים קרים', 'כף לימון כבוש קצוץ', 'חצי כפית חריף', 'מלח'], steps: ['פותחים טחינה עם מים קרים עד מרקם חלק.', 'מוסיפים לימון כבוש וחריף.', 'מתקנים מלח ומגישים קר.'] },
      { title: 'רוטב פלפלים קלויים', description: 'רוטב מתקתק-חרוך שמתאים למנגל פחמים.', ingredients: ['2 פלפלים קלויים', 'שן שום', '2 כפות שמן זית', 'כפית חומץ', 'מלח גס'], steps: ['קולפים פלפלים קלויים.', 'טוחנים עם שום, שמן וחומץ.', 'מתבלים ומגישים לצד פרוסות הבשר.'] },
    ],
    [
      { title: 'סלסה ירוקה חריפה', description: 'רוטב ירוק, חד ומהיר לכנפיים, פרגית וסטייקים.', ingredients: ['כוס עשבי תיבול', 'פלפל ירוק חריף', 'מיץ מחצי לימון', '2 כפות שמן זית', 'מלח'], steps: ['קוצצים או טוחנים גס את כל הרכיבים.', 'טועמים ומאזנים לימון ומלח.', 'מגישים מיד כדי לשמור על צבע ורעננות.'] },
      { title: 'גלייז תמרים וצ׳ילי', description: 'ציפוי מבריק, מתוק וחריף לשלב הסיום.', ingredients: ['2 כפות סילאן', 'כף סויה או מלח לפי כשרות', 'כפית צ׳ילי', 'כף מים חמים', 'מעט לימון'], steps: ['מערבבים את הרכיבים עד רוטב חלק.', 'מברישים רק בדקות האחרונות כדי שלא יישרף.', 'מגישים עוד קצת בצד.'] },
    ],
  ];
  const prepTime = isKebabLamb ? '45-75 דקות' : isQuick ? '45-60 דקות' : effort === 'מאוזן' ? '2-3 שעות' : '5-7 שעות';
  const difficulty = isKebabLamb ? 'בינוני' : isQuick ? 'קל' : effort === 'מאוזן' ? 'בינוני' : 'מתקדם';
  const primaryIngredient = isKebabLamb ? 'בשר טלה טחון גס - 720 גרם' : `${cleanCut} - ${portionForCut(cleanCut)}`;
  ...
  return {
    ...
    ingredients: [
      primaryIngredient,
      ...buildSeasoningIngredients(seasoningStyle, cleanCut),
      ...buildMethodIngredients(cleanCut, method, kosherPreference),
    ],
    ...
    sideDishes: pickSideDishes(cleanCut, method, seasoningStyle) ?? sideSets[seed % sideSets.length],
    sauces: pickSauces(cleanCut, method, kosherPreference) ?? sauceSets[(seed + method.length) % sauceSets.length],
  };
}
...
function buildCutAwareSteps(cut, method, seasoningStyle, kosherNote) {
  ...
  const seasoningReference = `רכיבי התיבול המפורטים ברשימת המצרכים (${seasoningStyle})`;
...
function buildSeasoningIngredients(style, cut) {
  if (cut.includes('קבב טלה')) {
...
function buildMethodIngredients(cut, method, kosherPreference) {
  if (cut.includes('קבב טלה')) {
...
function pickSideDishes(cut, method, style) {
  if (cut.includes('קבב טלה')) {
    return [
...
function pickSauces(cut, method, kosherPreference) {
  if (cut.includes('קבב טלה')) {
    return [
