...
export async function generateRecipeWithAi(req) {
  ...
  try {
    const output = await createOpenAiResponse({
      ...
      input: `צור מתכון מפורט לפי הפרטים הבאים: ${JSON.stringify(req)}. אם השיטה היא מעשנה, פרט חום, עשן, זמן, סיבוב או היפוך, עטיפה ומנוחה. אם זו גריל, מנגל ישראלי או פלנצ'ה, פרט אזור חם, אזור עקיף, צריבה, היפוך ומנוחה. אם זו קדירה, פרט צריבה, נוזלים, חום, זמן ובדיקת רכות. הקפד על העדפת הכשרות שנשלחה.`,
      text: {
        format: {
          type: 'json_schema',
