...
export async function generateRecipeWithAi(req) {
  ...
  try {
    const output = await createOpenAiResponse({
      ...
      input: `צור מתכון אמיתי ומותאם, לא תבניתי, לפי הפרטים הבאים: ${JSON.stringify(normalizedReq)}. התאם לנתח, לשיטת הבישול, לרמת ההשקעה, לכשרות ולסגנון התיבול. במצרכים חובה לתת כמויות ומשקלים, ובמצרך הראשון כתוב רק את שם הנתח שקונים מהקצב עם משקל. השתמש בשם הנתח המדויק בכל מקום חשוב. אם זו מעשנה, פרט חום, עשן, זמן, סיבוב או היפוך, עטיפה ומנוחה. אם זו גריל, מנגל ישראלי, גריל פחמים או פלנצ'ה, פרט אזור חם, אזור עקיף, צריבה, היפוך ומנוחה; בגריל פחמים הסבר שזה מנגל עם גחלים לוחשות ולא אש גלויה. אם זו קדירה, פרט צריבה, נוזלים, חום, זמן, בדיקת רכות והגשה עם הרוטב. החזר רטבים כאובייקטים עם שם, הסבר, רכיבים ושלבי הכנה. אל תחזור על אותן תוספות ורטבים אם אינם מתאימים לפרטים שנשלחו.`,
      text: {
        format: {
          type: 'json_schema',
